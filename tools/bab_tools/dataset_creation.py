...
from tools.bab_tools.model_utils import load_cifar_oval_kw_1_vs_all
import time, copy, os, argparse, math, csv
import torchvision.datasets as datasets
import torchvision.transforms as transforms
from adv_exp.mi_fgsm_attack import MI_FGSM_Attack


def run_root_planet_lb(domain, cuda_verif_layers):
    # Run KW/CROWN IB + 10 alpha-CROWN iterations on the CIFAR10 datasets from VNN-COMP20.
    ...
    prop_net = Propagation(cuda_verif_layers, type="alpha-crown", params=prop_params, store_bounds_primal=True)
    with torch.no_grad():
...
def run_quick_attacks(data, model, targeted=True):
    start_time = time.time()
    ...
    if not targeted:
        # performed non-targeted CE attacks
        restarts = 4
        for ctd in range(restarts):
            adv_examples, is_adv = adv_model.create_adv_examples(data, model, return_criterion='one', gpu=True)
...
def create_oval21(seed, n_splits=1, modulo_do=0, to_vnnlib=True):
    """
    ...
    if to_vnnlib:
        ...
        for onnx_name, nn_name, max_solv_batch in zip(onnx_names, nn_names, max_solver_batch_list):
            x, y, th_model, _ = load_cifar_oval_kw_1_vs_all(
                nn_name, 0, max_solver_batch=max_solv_batch, no_verif_layers=True, cifar_test=cifar_test)
    ...
    for counter, (nn_name, max_solv_batch, img_ids) in enumerate(zip(nn_names, max_solver_batch_list, images_indices)):

        # Divide the three datasets into blocks (not very fine-grained...)
        if n_splits > 1 and counter % n_splits != modulo_do:
            continue

        # Pandas table associated to new dataset.
        new_dataset_name = path + nn_name.split("_")[1] + "OVAL_WK.pkl"
        if os.path.isfile(new_dataset_name):
            new_dataset = pd.read_pickle(new_dataset_name)
        else:
            new_dataset = pd.DataFrame(index=list(range(n_images_per_net)), columns=["Idx", "Eps", "prop"])
            new_dataset.to_pickle(new_dataset_name)

        for idx in range(n_images_per_net):

            # Skip already found epsilons.
            if pd.isna(new_dataset.loc[idx]['Eps']) == False:
            ...
            def lb_from_epsilon(epsilon_value):
                ...
                bab_out, runtime = run_root_planet_lb(domain, cuda_verif_layers)
                return bab_out, runtime

            def ub_from_epsilon(epsilon_value):
                x, y, model, domain = load_cifar_oval_kw_1_vs_all(
                    nn_name, int(imag_idx), epsilon=epsilon_value, max_solver_batch=max_solv_batch,
                    cifar_test=cifar_test, no_verif_layers=True)
                # Move to GPU.
                model = model.cuda()
                x = x.cuda()
                domain = domain.cuda()
                ...
                # --------- Run attacks
                bab_out, runtime = run_quick_attacks(data, model)
                return bab_out, runtime
            ...
            # Record the results: pickled pandas table.
            new_dataset.loc[idx]["Idx"] = imag_idx
            new_dataset.loc[idx]["Eps"] = found_epsilon
            new_dataset.loc[idx]["prop"] = prop_idx
            new_dataset.to_pickle(new_dataset_name)
            if to_vnnlib:
...
if __name__ == "__main__":
    ...
    # CUDA_VISIBLE_DEVICES=0 taskset -c 0-3 python tools/bab_tools/dataset_creation.py --modulo 2 --modulo_do 0
    # CUDA_VISIBLE_DEVICES=1 taskset -c 4-7 python tools/bab_tools/dataset_creation.py --modulo 2 --modulo_do 1
    ...
    create_oval21(args.seed, n_splits=args.modulo, modulo_do=args.modulo_do)
